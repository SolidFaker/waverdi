...
fn toolbar_rects(tb: Rect) -> Vec<(Tool, Rect)> {
    let mut x = tb.x + 1;
    let mut out = Vec::with_capacity(TOOLS.len());
    for (name, tool) in TOOLS {
        let width = (name.len() + 4) as u16;
        out.push((
            tool,
            Rect {
                x,
...
pub fn tool_at(tb: Rect, col: u16) -> Option<Tool> {
    toolbar_rects(tb)
        .into_iter()
...
pub fn draw(buf: &mut Buffer, l: &Layout, app: &App) {
    ...
    for (tool, rect) in toolbar_rects(l.toolbar) {
        let name = TOOLS
            .iter()
            .find(|(_, t)| *t == tool)
            .map(|(name, _)| *name)
        ...
        text::put(buf, rect.x + 1, rect.y, name, style);
